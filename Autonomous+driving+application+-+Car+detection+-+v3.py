...
    """
    ...
    union_area = max(box1_area + box2_area - inter_area, .000001)
...
# Test YOLO pretained model on images
sess = K.get_session()
...
# Load pre-trained model (YoloV2) from H5
# which loads the weights of a trained YOLO model
yolo_model = load_model("yolo.h5")

# Print a summary of the layers your model contains.
yolo_model.summary()

# Convert the output of the model to usable bounding box tensors
# the output of the yolo model is a [m,19,19,5,85] tensor that needs to be converted to
# box_confidence, box_xy, box_wh, box_class_probs
yolo_outputs = yolo_head(yolo_model.output, anchors, len(class_names))

# Call yolo eval to convert the output of YOLO encoding (a lot of boxes) to your predicted boxes along
